...
public class PropertyController {
   ...
   @GetMapping// Δεν ξέραμε ακριβώς τι PreAuthorized να βάλουμε καθώς γίνεται μέσα στην μέθοδο γίνεται διαχείριση και tenant και owner και ο έλεγχος για το τι θα βλέπει ο καθένας γίνεται μέσα στο template, αφού πρόκειται για το ίδιο url
   public String showPropertiesPage(Model model) { // Method for checking on a new user
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'TENANT')")//Tenant or Admin have access to this resource
    ...
    @PreAuthorize("hasRole('ADMIN')")//Only admin can approve a property applic
    ...
    @PreAuthorize("hasRole('ADMIN')") //Only admin can reject a property applic
    ...
    @PreAuthorize("hasRole('OWNER')") //Only Owners can add a new property and property_applic
    @GetMapping("/new")
